...
package com.digiapp.jilmusic.playback;
...
public class QueueManager {
    ...
    public void updateMetadata() {
        ...
        if(currentMusic.getDescription().getMediaId()!=null
                && currentMusic.getDescription().getMediaId().startsWith(MEDIA_ID_MUSICS_BY_SELECTION)){
            musicId = MediaIDHelper.extractMusicIDFromSelection(currentMusic.getDescription().getMediaId());
        }else {
            musicId = MediaIDHelper.extractMusicIDFromMediaID(
                    currentMusic.getDescription().getMediaId());
        }

        MusicTrack musicTrack = mMusicProvider.getMusic(musicId);

        if(musicTrack==null){
            throw new IllegalArgumentException("Invalid musicId " + musicId);
        }

        MediaMetadataCompat metadata = musicTrack.item;
        ...
        if (metadata.getDescription().getIconBitmap() == null &&
            ...
            String finalMusicId = musicId;
            AlbumArtCache.getInstance().fetch(albumUri, new AlbumArtCache.FetchListener() {
                @Override
                public void onFetched(String artUrl, Bitmap bitmap, Bitmap icon) {

                    mMusicProvider.updateMusicArt(finalMusicId, bitmap, icon);
                    ...
                    String currentPlayingId = MediaIDHelper.extractMusicIDFromMediaID(
                            currentMusic.getDescription().getMediaId());
                    if (finalMusicId.equals(currentPlayingId)) {
                        mListener.onMetadataChanged(mMusicProvider.getMusic(currentPlayingId).item);
                    }
                }
            });
        }
