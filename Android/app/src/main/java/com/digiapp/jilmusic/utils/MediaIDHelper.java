...
public class MediaIDHelper {
    ...
    public static final String MEDIA_ID_FAVORITE = "__FAVORITE__";
    public static final String MEDIA_ID_MUSICS_BY_GENRE = "__BY_GENRE__";
    public static final String MEDIA_ID_MUSICS_BY_SELECTION = "__BY_SELECTION__";
    ...
    public static String createMediaID(String musicID, String... categories) {
        ...
        String root = categories[0];

        if (root.equalsIgnoreCase(MEDIA_ID_MUSICS_BY_SELECTION)) {
            if (categories != null) {
    ...
    public static String extractMusicIDFromMediaID(@NonNull String mediaID) {
        int pos = mediaID.indexOf(mediaID.startsWith(MEDIA_ID_MUSICS_BY_SELECTION)?CATEGORY_SEPARATOR:LEAF_SEPARATOR);
        if (pos >= 0) {
    ...
    public static String extractMusicIDFromSelection(@NonNull String mediaID){
    ...
    String[] getHierarchy(@NonNull String mediaID) {
        ...
        String[] category = mediaID.split(String.valueOf(CATEGORY_SEPARATOR));
        if (category.length == 1) {
            result[0] = category[0];
            result[1] = original.substring(pos + 1);
        } else {
            result = category;
        }
    ...
    public static boolean hasLeafSeparator(@NonNull String mediaID) {
        return mediaID.indexOf(LEAF_SEPARATOR) > 0;
    }
    ...
    public static long getSelectionId(String id){


        if(id==null
                || id.isEmpty()
                || !id.startsWith(MEDIA_ID_MUSICS_BY_SELECTION)){
            return -1;
        }

        if(id.indexOf(CATEGORY_SEPARATOR)<0){
            return -1;
        }
        ...
        long res = -1;
        try {
            res = Long.parseLong(id.substring(id.indexOf(CATEGORY_SEPARATOR) + 1));
    ...
    public static File getPicsDirectory(Context context) {

        ContextWrapper cw = new ContextWrapper(context);
        File directory = cw.getDir("pics", Context.MODE_PRIVATE);
        if (!directory.exists()) {
            directory.mkdir();
        }
        /*String dcim = android.os.Environment.DIRECTORY_DCIM;
        File directory = new File(dcim + "/offline");
        if (!directory.exists()) {
            directory.mkdir();
        }*/
