...
import com.digiapp.jilmusic.VoiceSearchParams;
...
import static com.digiapp.jilmusic.MusicService.mMusicProvider;
...
import static com.digiapp.jilmusic.utils.MediaIDHelper.MEDIA_ID_MUSICS_BY_SEARCH;
...
public class QueueHelper {
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueue(String mediaId,
        ...
        if (categoryType.equals(MEDIA_ID_MUSICS_BY_GENRE)) {
            tracks = musicProvider.getMusicsByGenrePrepared(categoryValue);
        } else if (categoryType.equals(MEDIA_ID_MUSICS_BY_SEARCH)) {
            tracks = musicProvider.searchMusicBySongTitle(categoryValue);
        } else if (categoryType.equalsIgnoreCase(MEDIA_ID_FAVORITE)) {
            tracks = musicProvider.getMusicsFavoriteForQueue(categoryValue);
        } else if (categoryType.equalsIgnoreCase(MEDIA_ID_MUSICS_BY_SELECTION)) {
            try {
                tracks = musicProvider.getMusicsBySelectionPrepared(Long.parseLong(categoryValue));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
    ...
    public static void updateQueue(String MediaIdcategory,List<MusicTrack> list){
        if(mMusicProvider == null){
            return;
        }

        if(MediaIdcategory.equalsIgnoreCase(MEDIA_ID_MUSICS_BY_GENRE)){
            mMusicProvider.mMusicListByGenre.remove(MEDIA_ID_MUSICS_BY_GENRE);
            mMusicProvider.mMusicListByGenre.put(MEDIA_ID_MUSICS_BY_GENRE,list);
        }
    }
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueueFromSearch(String query,
        ...
        if (params.isAlbumFocus) {
            ...
            result = musicProvider.getMusicsByGenrePrepared(params.genre);
    ...
    private static List<MediaSessionCompat.QueueItem> convertToQueue(
        ...
        for (MediaMetadataCompat track : tracks) {
            ...
            if (categories[0].equalsIgnoreCase(MEDIA_ID_MUSICS_BY_SELECTION)) {
                hierarchyAwareMediaID = categories[0] + "/" + track.getDescription().getMediaId();
            } else {
    ...
    public static List<MediaSessionCompat.QueueItem> getRandomQueue(MusicProvider musicProvider) {
        ...
        // preparing shuffled music
        Iterable<MusicTrack> musicTracks = musicProvider.getShuffledMusic();
        ArrayList<MediaMetadataCompat> tmp = new ArrayList<MediaMetadataCompat>();
        for (MusicTrack musicTrack : musicTracks) {
            tmp.add(musicTrack.item);
        }
        // preparing shuffled music

        Iterable<MediaMetadataCompat> shuffled = tmp;
