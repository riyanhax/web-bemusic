 ...
 */
public class MusicProvider_bak {

    private static final String TAG = LogHelper.makeLogTag(MusicProvider_bak.class);
    ...
    public MusicProvider_bak() {
        this(new RemoteJSONSource());
    }

    public MusicProvider_bak(MusicProviderSource source) {
        mSource = source;
    ...
    private synchronized void retrieveMediaDB(){
        try {
            if (mCurrentState == State.NON_INITIALIZED) {
                ...
                for(MusicTrack musicTrack:list){
                    ...
                    try {
                        musicTrack.item = RemoteJSONSource.buildFromFile(musicTrack.localPath);
                    }catch (Exception ex){
                        ex.printStackTrace();
                        continue;
                    }
                    ...
                    try {
                       // String musicId = musicTrack.item.getString(MediaMetadataCompat.METADATA_KEY_MEDIA_ID);
                       // mMusicListById.put(musicId, new MutableMediaMetadata(musicId, musicTrack.item));
                    } catch (Exception ex) {
                        ex.printStackTrace();
                        continue;
                    }
