...
export class NewTrackModalComponent {
     ...
     * Album this track should be attached to.
     */
    public rolestotrack: RolesToTrack;
    public mykey;
    ...
    /**
     * Model for rolesToTrack input and tags.
     */
    public rolesInput = {
        formControl: new FormControl(),
    ...
    /**
     * Attach a new artist role to track.
     */
    public attachRole(event: MatAutocompleteSelectedEvent) {
        const roleName = event.option.value;
        this.rolesInput.formControl.setValue('');

        if ( ! roleName) return;

        //make sure artist role is not already attached to track
        if (this.rolesInput.attachedRoles.findIndex(curr => curr === roleName) > -1) return;
        this.rolesInput.attachedRoles.push(roleName);
        this.errors = {};
    }

    /**
     * Detach specified artist role from track.
     */    
    public detachRole(rolestotrack: string) {
        let i = this.rolesInput.attachedRoles.findIndex(curr => curr === rolestotrack);
        this.rolesInput.attachedRoles.splice(i, 1);
    }

    public detachRoleName(rolestotrack: string, role:string ) {        
        let i = this.rolesInput.attachedRoles.findIndex(curr => curr === rolestotrack);
        let j = this.rolesInput.attachedRoles[i]['roles'].findIndex(curr => curr === role);
        this.rolesInput.attachedRoles[i]['roles'].splice(j, 1);
    }
    ...
    private getPayload() {
        ...
        let test = JSON.parse(JSON.stringify(this.rolesInput.attachedRoles));        
        test.map(key => {                        
             key['roles'] = key['roles'].join('*|*') as any;            
        });
        payload.rolestotrack = test;
    ...
    private hydrate(params: CrupdateTrackModalData) {
        ...
        //hydrate rolestotrack input model
        this.rolestotrack = Object.assign({}, params.rolestotrack, params.track ? params.track.rolestotrack : {});
        
        //this.rolesInput.attachedRoles = this.rolestotrack as any;
        this.rolesInput.attachedRoles = Object.keys(this.rolestotrack).map(key => {
            return this.rolestotrack[key];
        })
     ...
     */
    public albumDisplayFn(album?: Album|Artist|RolesToTrack|string): string {
        if ( ! album) return '';
    ...
    /**
     * Function for rolestotrack autocomplete input.
     */
    public rolestotrackDisplayFn(rolestotrack?: RolesToTrack|string): string {
        if ( ! rolestotrack) return '';
        
        if (typeof rolestotrack === 'string') {
            return rolestotrack;
        } else {
            return rolestotrack[0].name
        }
    }
    ...
    private bindRolesInput() {
        
        this.rolesInput.searchResults = this.rolesInput.formControl.valueChanges
            .pipe(
                distinctUntilChanged(),
                debounceTime(350),
                startWith(''),
                switchMap(query => {
                    const results = this.search.everything(this.rolestotrackDisplayFn(query), {limit: 5})
                        .pipe(map(results => results.artists));
