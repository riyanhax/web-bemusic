...
export class NewArtistPageComponent implements OnInit {
    ...
    public fethNewInfo(artistName: string) {  
        this.loading = true;

        this.artists.fethNew(artistName).toPromise().then(artist => {                        
            this.artist = artist;
            
            this.bio.text = artist.bio.bio;

            if ( ! artist.bio.images) artist.bio.images = [];
            this.bio.images = artist.bio.images.map(image => {
                return image.url;
            }).join("\n");

            this.toast.open('Artist fetch new info and updated.');
            this.errors = {};
            this.loading = false;
        }).catch(() => {
            this.loading = false;
            this.toast.open('Artist fetch new info failed.');
        }) as any;
