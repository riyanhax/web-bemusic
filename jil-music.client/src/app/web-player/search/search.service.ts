...
export class Search {
    ...
    public everythingAdmin(query: string, params?: {limit: number, type?: string}): Observable<SearchResponse> {
        return this.http.get('search-admin/'+encodeURIComponent(query), params);
    }

    public everythingFetch(query: string, params?: {limit: number, type: string}): Observable<SearchResponse> {
        return this.http.get('search-fetch/'+encodeURIComponent(query), params);
    }
