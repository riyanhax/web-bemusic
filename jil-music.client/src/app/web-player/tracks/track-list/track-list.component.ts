...
export class TrackListComponent implements OnInit, OnChanges, OnDestroy {
    ...
    public clickToAndroid(track: Track, type: string, e: MouseEvent) {
        e.stopPropagation();
        e.preventDefault();
        if(this.isAndroid){
            const data = {track: track, type: type, lyric: ''};
            if(type == 'lyric'){
    ...
    public fetchLyrics(data) {              
        return this.lyrics.get(data.track.id).toPromise().then(lyric => {                
    ...
    public sendAndroid(data) {        
        Android.clickTrackBtn(JSON.stringify(data));
    }
