...
import {UserSubscriptionsModalComponent} from './user-subscriptions-modal/user-subscriptions-modal.component';
...
export class UsersComponent implements OnInit, OnDestroy {
    ...
    /**
     * Show modal for user subscriptions.
     */
    public showUserSubscriptionsModal(user?: User) {
        this.modal.open(
            UserSubscriptionsModalComponent,
            {user},
            'user-subscriptions-modal-container'
        ).afterClosed().subscribe(data => {
            //if ( ! data) return;
            //this.paginator.refresh();
        });
     ...
     */
    public filter(value) {
        this.dataSource.setParams({subscription: value});
    }
}
